...
export const getCommitHashes = async (githubUrl: string, total: number): Promise<GithubCommit> => {
    ...
    return sortedCommits.slice(0,total).map((commit:any) => ({
 ...
 export const pollCommits = async (projectId: string, total: number) =>{
    const { githubUrl} = await fetchProjctGithubUrl(projectId)
    const commitHashes:any = await getCommitHashes(githubUrl,total);
